...
import type { ObjectProcessorMap } from 'webgi/helpers/ObjectProcessorMap';
import type { IUniform, TextureDataType, TextureEncoding, TextureFilter, Vector4Tuple, WebGLRenderTargetOptions } from 'three';
 ...
 * Scene
 */
export interface AddModelOptions extends AnyOptions {
    autoScale?: boolean;
...
export interface IScene<TScene = any, TCamera extends ICamera<ICameraOptions> = ICamera<ICameraOptions>> extends IEventDispatcher<'sceneUpdate' | 'addSceneObject' | 'update' | 'textureAdded' | 'activeCameraChange' | 'activeCameraUpdate' | 'sceneMaterialUpdate' | 'environmentChanged'>, IDisposable, IUiConfigContainer, IShaderPropertiesUpdater {
    assetType: 'model';
    modelObject: TScene;
    ...
    setEnvironment(asset: ITexture | null | undefined): Promise<void>;
    getEnvironment(): ITexture;
    setBackground(asset: ITexture | string | number | null | any): Promise<void>;
...
export declare type TCameraControlsMode = 'orbit' | '';
...
export interface ICamera<TOptions extends ICameraOptions, TCamera = any, TControls = any> extends IModel, IShaderPropertiesUpdater, IJSONSerializable {
    near: number;
    far: number;
    setDirty(): void;
    activateMain(): void;
 ...
 * Others
 */
export interface IModel<TModel = any> extends IEventDispatcher<string>, IDisposable, IUiConfigContainer {
    assetType: 'model';
    modelObject: TModel;
    material?: IMaterial;
}
export interface ILight<TLight = any> extends IEventDispatcher<string>, IDisposable, IUiConfigContainer, IJSONSerializable {
    assetType: 'light';
    lightObject: TLight;
    modelObject: TLight;
}
export interface IMaterial<TMaterial = any> extends IEventDispatcher<string>, IUiConfigContainer, IJSONSerializable {
    assetType: 'material';
    name: string;
    uuid: string;
 ...
 * Utils
 */
export interface ITextureUtil extends IEventDispatcher<void> {
    instance: ITextureUtil;
}
export interface IModelUtil extends IEventDispatcher<void> {
    instance: IModelUtil;
}
export interface IMaterialUtil extends IEventDispatcher<void> {
    instance: IMaterialUtil;
}
/**
 * Viewer
 */
...
export interface IViewerApp extends IEventDispatcher<'update' | 'preRender' | 'postRender' | 'preFrame' | 'postFrame' | 'dispose' | 'addPlugin'>, IJSONSerializable {
    ...
    setBackgroundColor(color: string | number | any): Promise<void>;
    setBackground(asset: ITexture | string | number | null): Promise<void>;
